...
To learn and practice how to organise a data we will use a `gapminder` data set awailable from `gapminder` package in R. This dataset is put into R by [Jennifer Bryan](https://jennybryan.org/) from a tank od data sets available from [Gapminder](https://www.gapminder.org).

[Gapminder](https://www.gapminder.org) is an independent Swedish foundation that helps to promote sustainable global development by collecting and analysiing relevan data and by developing and designing  teaching/learning tools. [Gapminder](https://en.wikipedia.org/wiki/Gapminder_Foundation) was founded in Sweden by [Hans Rosling](https://en.wikipedia.org/wiki/Hans_Rosling) who was a mistermind for ditinctive and uterlly insightful storytelling about global development using visual animation.
...
#
# have a look at the data trough a table
...
Can you tell what does each of the two function do⁉️
Do we have the inormation about the structure of the data? 🤔 We can examine the structure using <span style="color:red">`str()`</span> function, but the **output could look messy** and hard to follow if the data set is big. 🤪
...
Rhe <span style="color:red">**`dplyr`**</span> provides a “grammar” (the verbs) for data manipulation and for operating on data frames in a tidy way. The key opertor and the esential verbs are :
...
<span style="color:red">summarise() / summarize()</span>: reduces each group to a single row by calculating aggregate measures.

We can have a look at the data and its structure by using the <span style="color:red">`glimpse()`</span> function from `diplyr` package.
...
Now we have `diplyr` package uploaded, let us lear its verbs. 😇
...
You can try to get into a habit of using a shortcut for the pipe operatir 
...
**Look what does this code produce**
...
1) Install and upload `rattle` package and explore what does it do.

2) Create a new R script file to explore `weatherAUS` dataset. 

  i) `select()` variable: `MinTemp`, `MaxTemp`, `Rainfall` and `Sunshine` by *pyping* the dataset into `dplyr::select() function.
  
  ii) produce a *summary*  using `base::sumarry()` function of these numeric values.
  
  iii) within this selection filter only those observations where `Rainfall >= 1` and save the results into the computers memory (ie. save the results as an object).
  ...
  v) Write a short report on what visualisation do you think would be interesting to produce for this `weatherAUS` dataset and why?

##### Useful Links

[Data Wrangling cheet sheat](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf)
