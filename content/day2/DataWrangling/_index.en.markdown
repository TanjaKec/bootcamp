...
To learn and practice how to organise a data we will use a `gapminder` data set awailable from `gapminder` package in R. This dataset is put into R by [Jennifer Bryan](https://jennybryan.org/) from a tank of data sets available from [Gapminder](https://www.gapminder.org).
...
Can you tell what does each of the two functions do⁉️
...
Rhe <span style="color:red">**`dplyr`**</span> provides a “grammar” (the verbs) for data manipulation and for operating on data frames in a tidy way. The key opertor and the essential verbs are:
...
Now we have `diplyr` package uploaded, let us learn its verbs. 😇
